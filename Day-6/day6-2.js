...
// Recursivly solves the problem. Returns distance between first matching state
solve2 = (iterations) => {
    iterations.push(spread(iterations[iterations.length-1].slice(0)))
    if(hasDuplicates(iterations)) {
        iterations = iterations.map(v=>v+'')
        return iterations.length - iterations.indexOf(iterations[iterations.length-1]) - 1
    }
